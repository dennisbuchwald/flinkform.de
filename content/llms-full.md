# Flinkform - Volltexte der Kernseiten
...
Funktioniert Flinkform mit meinem Theme? Ja. Flinkform liest die Design-Tokens aus theme.json und übernimmt Farben, Typografie, Abstände und Eckenradius automatisch. Getestet mit GeneratePress, Twenty Twenty-Five, Astra und Kadence.

---

## Seite: Flinkform Pro (https://flinkform.de/pro)
...
| Preis Free | 0 € |
| Preis Pro | 59/99/149/299 €/Jahr (1/3/25/unbegrenzt Websites), Lifetime-Launch 399 € |
| Hersteller | Dennis Buchwald, dbw media, Heilbronn (Deutschland) |
...
## Wissens-Artikel (Kurzantworten)

### Was ist ein block-natives Formular-Plugin? (https://flinkform.de/wissen/was-ist-ein-block-natives-formular-plugin)

Ein block-natives Formular-Plugin ist ein WordPress-Plugin, bei dem das Formular direkt aus Gutenberg-Blöcken im Block-Editor besteht: Jedes Feld ist ein eigener Block, das Formular wird an Ort und Stelle in der Seite gebaut. Es gibt keinen separaten Formular-Builder, kein Shortcode-Embed und keine eigene Admin-Oberfläche für den Formularbau. Bekannte Vertreter sind Flinkform und SureForms; Gegenbeispiele sind Contact Form 7 (Shortcode) sowie WPForms und Gravity Forms (eigener Builder).

### WordPress-Formular ohne reCAPTCHA (https://flinkform.de/wissen/wordpress-formular-ohne-recaptcha)

Ein WordPress-Formular lässt sich ohne Google reCAPTCHA zuverlässig vor Spam schützen: mit einer Kombination aus Honeypot, signiertem Zeit-Check und Proof-of-Work, die komplett auf dem eigenen Server läuft. Diese Mechanismen brauchen keinen externen Dienst, setzen keine Cookies und erfordern keine Einwilligung. Das kostenlose Plugin Flinkform hat alle drei eingebaut.

### Welches WordPress-Formular-Plugin ist DSGVO-konform? (https://flinkform.de/wissen/dsgvo-konformes-formular-plugin)

Kein Formular-Plugin ist automatisch DSGVO-konform, die Voraussetzungen unterscheiden sich aber stark: Entscheidend ist, ob das Plugin ohne US-Dienste auskommt (v. a. Spam-Schutz), ob es IP-Adressen speichert und ob Consent-Feld, Löschfristen und Privacy-Tools eingebaut sind. Nach diesen Kriterien schneidet Flinkform am besten ab: Es arbeitet als einziges verbreitetes Plugin standardmäßig komplett ohne externe Dienste und ohne IP-Speicherung.

### Gutenberg-Formular erstellen ohne Page Builder (https://flinkform.de/wissen/gutenberg-formular-erstellen)

Ein Formular direkt im Gutenberg-Editor braucht weder Elementor noch einen separaten Formular-Builder: Mit einem block-nativen Plugin wie Flinkform fügst du einen Form-Block ein, ergänzt Feld-Blöcke und veröffentlichst. Ein einfaches Kontaktformular steht in unter 5 Minuten.

### SEPA-Zahlung im WordPress-Formular annehmen (https://flinkform.de/wissen/sepa-zahlung-wordpress-formular)

SEPA-Lastschrift lässt sich direkt im WordPress-Formular annehmen: über das Stripe Payment Element (in Flinkform Pro), das SEPA neben Kreditkarte, Apple Pay und Google Pay anzeigt. Besonderheit: Die SEPA-Bestätigung dauert Tage, deshalb arbeitet die Integration mit einem Status-Modell ("in Bearbeitung" bis "bezahlt") und einem Stripe-Webhook.

### Multi-Step-Formular in WordPress erstellen (https://flinkform.de/wissen/multi-step-formular-wordpress)

Ein Multi-Step-Formular entsteht in WordPress am einfachsten block-nativ: Mit Flinkform fügst du zwischen die Feld-Blöcke einen Page-Break-Block ein, wählst eine Fortschrittsanzeige (Balken, Punkte oder Zahlen) und bekommst Schritt-Validierung automatisch, kostenlos. Bei WPForms und Gravity Forms sind mehrseitige Formulare Teil der Bezahlpläne, bei SureForms des Bezahlplans.

### Was ist bedingte Logik in WordPress-Formularen? (https://flinkform.de/wissen/bedingte-logik-wordpress-formular)

Bedingte Logik (Conditional Logic) blendet Formularfelder, ganze Schritte oder den Absenden-Button abhängig von vorherigen Antworten ein oder aus. Das hält Formulare kurz und relevant. In Flinkform ist bedingte Logik kostenlos enthalten und wird pro Feld im Block-Inspector konfiguriert.

### Warum kommen meine WordPress-Formular-Mails nicht an? (https://flinkform.de/wissen/wordpress-formular-mails-kommen-nicht-an)

WordPress-Formular-Mails landen im Spam oder verschwinden, weil wp_mail() ohne Authentifizierung über den Webserver sendet. Seit Februar 2024 verlangen Gmail und Yahoo SPF, DKIM und DMARC. Die Lösung: DNS-Einträge setzen und den Versand über SMTP routen, zum Beispiel mit dem SMTP-Modul von Flinkform Pro (7 Provider-Presets, Sende-Log mit Fehlerdiagnose).

### Formular-Spam stoppen in WordPress (https://flinkform.de/wissen/formular-spam-stoppen-wordpress)

Formular-Spam lässt sich mit serverseitigen Methoden (Honeypot, signierter Zeit-Check, Proof-of-Work) oder externen Diensten (reCAPTCHA, hCaptcha, Turnstile, Akismet) stoppen. Für die meisten Websites reicht die Kombination der serverseitigen Methoden: wirksam, unsichtbar, ohne Einwilligungspflicht. Genau diese Kombination ist in Flinkform eingebaut.

### Geht ein WordPress-Formular ohne Plugin? (https://flinkform.de/wissen/wordpress-formular-ohne-plugin)

Technisch ja: eigenes HTML plus PHP-Handler im Theme. Praktisch müssen dann Validierung, CSRF-Schutz, Spam-Abwehr, Mail-Versand, Speicherung und DSGVO-Löschung selbst gebaut und gepflegt werden. Der WordPress-Core bringt keinen Formular-Block mit. Für fast alle Projekte ist ein schlankes Plugin die bessere Wahl.
...
Die Spanne reicht von 0 Euro (Flinkform, Contact Form 7) über 59 bis 599 US-Dollar pro Jahr (Gravity Forms, WPForms, Fluent Forms, SureForms, Ninja Forms, Formidable) bis zu SaaS-Monatsabos (Typeform, Jotform). Versteckte Kosten: Renewal-Preise nach dem Rabatt-Erstjahr, Transaktionsgebühren (WPForms: 3 Prozent auf Stripe-Zahlungen unterhalb des Pro-Plans) und Add-on-Modelle. Flinkform Pro kostet 59 bis 299 Euro pro Jahr, alle Pläne mit allen Features.

### Braucht ein Kontaktformular einen Cookie-Banner? (https://flinkform.de/wissen/cookie-banner-kontaktformular)

Ein Kontaktformular selbst braucht keinen Cookie-Banner. Einwilligungspflichtig wird es erst durch eingebettete Drittdienste wie reCAPTCHA, die Cookies setzen und Besucherdaten verarbeiten. Flinkform setzt nur ein einziges strikt notwendiges Cookie (flinkform_flash, rund 60 Sekunden, httpOnly, nur bei fehlgeschlagener Validierung) und braucht deshalb keinen Banner.
...
Ein Angebotsrechner im WordPress-Formular entsteht mit den Berechnungsfeldern von Flinkform Pro: Formeln wie Menge mal Stückpreis plus Zuschläge rechnen live im Browser, während der Besucher tippt, und werden serverseitig manipulationssicher nachgerechnet (kein eval). Die berechnete Summe dient als qualifizierte Angebotsbasis; die direkte Abbuchung berechneter Summen steht auf der Roadmap.

### Formular-Conversions tracken (https://flinkform.de/wissen/conversion-tracking-wordpress-formular)

Formular-Conversions misst man am zuverlässigsten über den Redirect auf eine Danke-Seite: Flinkform leitet nach dem Absenden weiter und hängt optional submission_id und form_id als Parameter an. Darauf lassen sich GA4-Schlüsselereignisse, Meta-Pixel-Lead-Events oder cookiefreie Ziele (Plausible, Matomo) aufsetzen.
...
## Weitere Blog-Artikel (Kernaussagen)
...
Ehrliches Roundup über neun Plugins mit offengelegten Kriterien und Transparenz-Hinweis (Flinkform ist das eigene Produkt). Empfehlung nach Anwendungsfall: DSGVO-saubere Standard-Formulare und Multi-Step kostenlos: Flinkform. Enterprise-Workflows: Gravity Forms. Maximale Vorlagen: WPForms. Umfragen/Quiz kostenlos: Forminator. Frontend-Views: Formidable.
...
wp_mail() sendet ohne Authentifizierung; seit Februar 2024 verlangen Gmail und Yahoo SPF, DKIM und DMARC von Absendern. Lösung: DNS-Einträge setzen und Formular-Mails per SMTP versenden, etwa über das SMTP-Modul von Flinkform Pro mit Sende-Log.

---

## Blog-Artikel (Kernaussagen)
...
Für Anzahlungen, Buchungen und Einzelverkäufe braucht es keinen Shop: Das Stripe Payment Element bringt Kreditkarte, SEPA, Apple Pay und Google Pay in ein Formularfeld. Eine saubere Integration bindet den Betrag serverseitig, verifiziert die Zahlung vor dem Speichern und fängt Doppel-Submissions ab. Flinkform Pro setzt alle drei Regeln um; Zahlungen laufen direkt auf das eigene Stripe-Konto.
